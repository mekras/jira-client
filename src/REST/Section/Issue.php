...
final class Issue extends Section
{
     ...
     * Constants are used as $expand parameters in get issue information methods (->get(),
     * ->search(), ...)
     *
    ...
    {
        $this->rawClient->put("issue/{$issue_key}/assignee", ['name' => $user_name]);
    }
    ...
    public function create(
        ...
        return $this->rawClient->post('/issue', $create_request);
    }

    public function delete(string $issue_key): void
    {
        $this->rawClient->delete("/issue/{$issue_key}");
    }
    ...
    public function edit(
        ...
        $this->rawClient->put("issue/{$issue_key}", $update_request);
    }
     ...
     *                         (check 'Expansion' section at
     *                         https://docs.atlassian.com/jira/REST/cloud/ for more info)
     * @param array  $properties
     *
     * @return \stdClass
     * @throws \Mekras\Jira\REST\Exception
    ...
    public function get(
        ...
        return $this->rawClient->get("issue/{$issue_key}", $args);
    }

    /**
     * Get metainformation about the issue creation for specific projects and issue types: fields
     * available on creation screen, their possible values and so on.
     *
     * @see https://docs.atlassian.com/software/jira/docs/api/REST/7.6.1/#api/2/issue-getCreateIssueMeta
     *
     * @param string|string[] $projects      - list of unique Jira Project keys or IDs (e.g. IOS or
     *                                       12345).
     * @param string|string[] $issue_types   - list of unique Jira issue type names or IDs (e.g.
     *                                       'Bug' or 12345)
    ...
    public function getCreateMeta($projects, $issue_types = [], bool $expand_fields = false): array
    {
        ...
        $response = $this->rawClient->get('issue/createmeta', $parameters);

        return $response->projects;
    }

    /**
     * Get metainformation about issue editing: what fields are available on edit screen, their
     * possible values, etc.
     *
    ...
    public function getTransitions(string $issue_key, bool $expand_fields = false)
    {
        ...
        $actions_info = $this->rawClient->get("issue/{$issue_key}/transitions", $request_data);
    ...
    public function search(
        ...
        $issues = $this->rawClient->post('/search', $args)->issues;
