...
final class Field extends Section
{
    /**
     * @var array
     */
    ...
    public function create(
        ...
        $this->rawClient->post('/field', $args);
     ...
     *
     * @param bool $reload_cache   - force data reload. The method caches fields list, you can
     *                             bypass cache and make it to load fresh data from API once again.
     *
    ...
    {
        if (!isset($this->fieldsList) || $reload_cache) {
            $this->fieldsList = [];

            $fields_list = $this->rawClient->get('/field');
            foreach ($fields_list as $field_info) {
                $is_custom = $field_info->custom;
                $field_id = $field_info->id;

                $this->fieldsList[$field_id] = $field_info;

                if ($is_custom) {
                    $this->customFields[$field_id] = $field_info;
                } else {
                    $this->systemFields[$field_id] = $field_info;
                }
            }
        }
    ...
    {
        if (!isset($this->customFields) || $reload_cache) {
    ...
    {
        if (!isset($this->systemFields) || $reload_cache) {
            $this->list($reload_cache);
        }
     ...
     * @param bool   $reload_cache   - force internal cache reload and request API for the fresh
     *                               data before search
     *
